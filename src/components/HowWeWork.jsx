...
const timelineData = steps.map((step) => ({
  ...
  content: (
    <div>
      <h4 className="font-display font-bold text-2xl md:text-3xl text-white mb-3">
        {step.title}
      </h4>
      <p className="text-base md:text-lg leading-relaxed text-white/75 mb-6 max-w-xl">
        {step.text}
      </p>
      <ul className="space-y-3">
        {step.bullets.map((b) => (
          <li key={b} className="flex items-start gap-3 text-sm md:text-base text-white/85">
